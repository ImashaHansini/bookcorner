...
class Converter extends React.Component {
    ...
    convertHandler = () => {
      if (this.state.fromCurrency !== this.state.toCurrency) {
        axios
        .get(
            `https://v6.exchangerate-api.com/v6/9fa0b1ed62cae823aadaff37/pair/${
    ...
    render() {
      return (
        <div className="Converter">
          <div className="From">
            <input class = "input-group d-flex mb-3"
              name="amount"
              type="text"
              value={this.state.amount}
              onChange={event => this.setState({ amount: event.target.value })}
            />
            <select class = "custom-select custom-select-sm d-flex mb-3 mr-3"
              name="from"
